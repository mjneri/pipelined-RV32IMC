...
lcd_setup:
	...
	c.addi sp, -8				# push lcd_pulse_en args to stack
	c.swsp s1, 0				# command
	c.swsp s3, 1				# slave address
	c.jal lcd_pulse_en			# pulse E
...
lcd_print:
	...
	c.li s6, 0					# initialize s6 to 0
	c.li s7, 16					# loop limit 1
	addi s8, x0, 32				# loop limit 2
	c.li s9, 0x1c				# s9 points to string in datamem
	...
	lcd_print_loop1:
		...
		c.slli s5, 4				# shift left 4 bits to D<7:4>
		...
		c.swsp s3, 1				# slave address
		c.jal lcd_pulse_en			# pulse E
		c.jal delay_100us			# 100us delay_20ms
...
i2c_send:
	...
	c.lwsp t0, 0				# get data to be sent
	c.lwsp t1, 1				# get # of bytes to be sent
	c.lwsp t2, 2				# get slave address
	...
	c.mv t3, t1					# copy t1 to t3
	c.slli t3, 11				# shift to #BYTES field
	c.mv t4, t2					# copy slave address to t4
...
lcd_pulse_en:
	...
	c.lwsp t0, 0				# get data to be sent
	c.lwsp t1, 1				# get slave address
	c.addi sp, 8				# pop stack

	ori t2, t0, 0x04			# assert E
	c.li t3, 1					# send 1 byte only
	
	c.addi sp, -24				# push to stack
	c.swsp t2, 0				# pass data to be sent to i2c_send
	c.swsp t3, 1				# pass #Bytes to i2c_send
	c.swsp t1, 2				# pass slave address to i2c_send
	c.swsp t0, 3				# preserve data passed to lcd_pulse_en
	c.swsp t1, 4				# preserve slave address
	c.swsp ra, 5				# store return address to stack
	c.jal i2c_send				# call i2c_send()
	c.jal delay_100us			# call delay_100us()

	c.lwsp t0, 0				# get preserved data back from stack
	c.lwsp t1, 1				# get preserved slave address
	c.addi sp, 8				# pop stack
